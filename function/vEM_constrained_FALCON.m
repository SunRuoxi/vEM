...
for frame_count = 1: ceil(numFrame/frame_subset_length)
   ...
   if inverse_lambda0
       ...
       w = para*sqrt(max(repmat(1./lambda0,[1,1,size(est_backgrounds,3)]),bg_min)); % !!!!!!!!!!!!!!!!!!!!!!!!!!! CHANGE ME
   else      
        w = para*sqrt(max(imresize(est_backgrounds,up_decon),bg_min));    %falcon    
   end
    ...
    if sum(ind_set(:))>0
       ...
       % [est_c,delta_x,delta_y] = Peakdets((est_c),thresh);
         [est_c,delta_x,delta_y] = Peakdets_range((est_c),thresh,range);
    ...
    index_Results = ee+1;
    back_est(:,:,sframe:eframe) = est_backgrounds; 
end
ind = Results(:,4) > 1;
